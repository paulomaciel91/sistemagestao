...
export default async function handler(req, res) {
  ...
  console.log('API gerar-link-pagamento iniciada', { method: req.method });
  ...
  try {
    console.log('Processando requisição para gerar link de pagamento');
    const { 
      lojaId,
      vendaId,
      clienteId,
    ...
    if (!asaasApiKey) {
      console.log('Erro de validação: chave de API Asaas ausente');
      return res.status(400).json({ 
        success: false,
